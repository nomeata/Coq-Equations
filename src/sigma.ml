...
let generalize_sigma (id, b, c as decl) env evm concl =
  ...
  let indargs, indpars = chop lenargs ctx in
  let pars, args = chop mib.mind_nparams_rec (Array.to_list args) in
  let parapp = mkApp (mkInd ind, extended_rel_vect 0 indpars) in
  let newargs = Array.init lenargs (fun i -> mkRel (lenargs - i)) in
  let fullapp = mkApp (mkApp (f, Array.of_list pars), newargs) in
  let vbinder = (Name (add_suffix (ind_name ind) "_var")), None, fullapp in
  let ctx = vbinder :: indargs in
  let evm = ref evm in
  let _, _, _, _, _, valsig, _ = sigmaize (push_rel_context indpars env) evm parapp in
  let left = valsig in
  let sig_pack = it_mkLambda_or_LetIn valsig ctx in
  let rightargs = Array.of_list (args @ [mkVar id]) in
  let right = mkApp (lift (lenargs + 1) sig_pack, rightargs) in
  ...
  let ctxpars = map2 (fun (n, _, t) c -> (n, Some c, t)) indpars (rev pars) in
  let concl = it_mkLambda_or_LetIn concl ctxpars in
  let concl = Tacred.simpl env evm concl in
